...
import subprocess
import sys
# Weather marketplace contract will be handled via generated clients
from collections.abc import Callable
from pathlib import Path
from shutil import rmtree

import algokit_utils
from smart_contracts.weather_marketplace.contract import WeatherMarketplace
...
def import_deploy_if_exists(folder: Path) -> Callable | None:
    ...
    match action:
        ...
        case "deploy":
            ...
            for contract in filtered_contracts:
                ...
                app_spec = algokit_utils.Arc56Contract.from_json(app_spec_path.read_text())
                
                if contract.deploy:
                    logger.info(f"Deploying app {contract.name}")
                    contract.deploy(algod_client, indexer_client, app_spec, deployer)
        case "all":
            # Set up clients for deployment if needed
            algod_client = None
            indexer_client = None
            deployer = None
